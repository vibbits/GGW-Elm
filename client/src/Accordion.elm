...
module Accordion exposing (accordion, head, body)

import Element exposing (..)

type alias Head msg = Element msg
type alias Body msg = Element msg

type alias Display = Bool
...
head attrs = row ([width fill] ++ attrs)

body : List (Element.Attribute msg) -> List (Element msg) -> Body msg
body attrs = column ([width fill] ++ attrs)