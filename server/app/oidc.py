...
async def token(
    ...
    config: Dict[str, str],
    token_request: Dict[str, Optional[str]],
) -> ResponseType:
...
async def userinfo(
    client: httpx.AsyncClient, config: Dict[str, str], access_token: str
) -> ResponseType:
...
def create_access_token(*, data: schemas.User) -> bytes:
...
def decode_state(encoded: str) -> Optional[OidcState]:
    "Decode a URL encoded state object"
    try:
        decoded = json.loads(base64.urlsafe_b64decode(encoded))
    except (JSONDecodeError, Base64DecodeError):
        return None
    else:
        return decoded
